		...
		if (mat < 26) {
			if (mat < 14) {
				if (mat < 8) {
					...
					#ifndef END
						if (mat == 3) return vec4(vec3(1.0, 1.0, 1.0) * 0.25, 0.0); // End Rod - This is the base for all lights. Total value 12
					#else
				...
				if (mat < 20) {
					...
					if (mat == 18) return vec4(vec3(1.0, 1.25, 1.5) * 2.4, 0.0); // Sea Lantern
					...
					if (mat == 25) return vec4(netherPortalSpecialLightColor * 1.5, 0.4); // Nether Portal
				}
			}
		} else {
			if (mat < 38) {
		...
		if (mat < 74) {
			if (mat < 62) {
				if (mat < 56) {
					...
					if (mat == 58) return vec4(vec3(0.0, 1.4, 1.4) * 1.0, 0.15); // End Portal Frame:Active
